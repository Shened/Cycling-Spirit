...
export async function GET() {
    ...
    for (const [groupKey, groupConfig] of Object.entries(ACTIVITY_GROUPS)) {
        ...
        groupStats[groupKey] = monthlyStats.map((m) => ({
            ...
            durationHours: Math.round(m.durationHours * 10) / 10,
        ...
        if (!hasGoal("distance_km", "monthly", groupKey) && avgDistance > 0) {
            suggestions.push({
                metric: "distance_km", period: "monthly",
                target: round(avgDistance * BOOST),
                year: currentYear, month: currentMonth, activityGroup: groupKey,
                ...
                boost: "+10%", groupEmoji: groupConfig.emoji, groupLabel: groupConfig.label,
            });
        }

        if (!hasGoal("duration_hours", "monthly", groupKey) && avgDuration > 0) {
            suggestions.push({
                metric: "duration_hours", period: "monthly",
        ...
        if (!hasGoal("activities_count", "monthly", groupKey) && avgActivities > 0) {
            suggestions.push({
                metric: "activities_count", period: "monthly",
        ...
        if (!hasGoal("distance_km", "annual", groupKey) && avgDistance > 0) {
            suggestions.push({
                metric: "distance_km", period: "annual",
                target: round(avgDistance * 12 * BOOST),
                year: currentYear, month: null, activityGroup: groupKey,
    ...
    // Determina o desporto dominante (mais atividades nos últimos 3 meses)
    let dominantGroup = "all";
    let maxActivities = 0;

    for (const [groupKey, stats] of Object.entries(groupStats)) {
        const total = (stats as { activitiesCount: number }[])
            .reduce((s, m) => s + m.activitiesCount, 0);
        if (total > maxActivities) {
            maxActivities = total;
            dominantGroup = groupKey;
        }
    }

    return NextResponse.json({ suggestions, groupStats, dominantGroup });
}